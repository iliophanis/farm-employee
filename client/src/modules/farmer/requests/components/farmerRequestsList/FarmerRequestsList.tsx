...
type IRequestsListProps = {
  ...
  setData: Dispatch<SetStateAction<IFarmerRequestItem[]>>;
  loading: boolean;
  userRequests: IFarmerRequestResponse;
  handleChangePage: () => void;
  handleDelRequest: (empReqId: number) => void;
  handleOpenDetails: (id: number) => void;
};
const FarmerRequestsList = ({
  isPersonalRequests,
  type,
  data,
  setData,
  ...
  return (
    ...
    >
      <>
        {data.map((d: IFarmerRequestItem, idx) => (
            ...
            <div className=''>
              ...
              <div className='rounded-lg bg-gray-100 p-4 shadow-lg'>
                ...
                <div className='mb-2 text-gray-700'>
                  {d.employeeRequests.length > 0 && (
                    <div className='flex flex-row items-center text-gray-900'>
                      Αιτούντες Υπάλληλοι :
                      <ul role='list' className='ml-2'>
                        {d.employeeRequests.map(
                          (s: EmployeeRequestDto, idx: number) => {
                            const subEmployees = s.subEmployees
                              .map((x) => `${x.name} (${x.email})`)
                              .join(',');
                            const label =
                              subEmployees !== ''
                                ? `${s.name} (${s.email}), ${subEmployees}`
                                : `${s.name} (${s.email})`;
                            return (
                              <li key={idx}>
                                <Badge label={label} />
                ...
                <div className='flex flex-row justify-end'>
                  ...
                  {isPersonalRequests && false && (
                    <Button
                      variant='red'
                      size='sm'
                      onClick={() => handleDelRequest(d.requestId)}
                    >
                      Διαγραφή
                    </Button>
                  )}
...
export default FarmerRequestsList;
